...
def _ordered_moves():
    """Ưu tiên cột trung tâm để PV search hội tụ nhanh hơn."""
    return [3, 2, 4, 1, 5, 0, 6]


def pvs(me, opp, depth, alpha, beta, deadline, tt):
    state = (me, opp)
    if state in tt:
        res, d = tt[state]
        if d >= depth:
    ...
    # PVS: nước đầu tìm full window, nước sau tìm null-window.
    for col in _ordered_moves():
        ...
        if first_child:
            res = -pvs(opp, me | new_piece, depth - 1, -beta, -alpha, deadline, tt)
            first_child = False
        else:
            # Null-window search: nhanh hơn khi move ordering tốt.
            res = -pvs(opp, me | new_piece, depth - 1, -alpha - 1, -alpha, deadline, tt)
            # Nếu node có tiềm năng tốt hơn alpha, re-search với cửa sổ đầy đủ.
            if alpha < res < beta:
                res = -pvs(opp, me | new_piece, depth - 1, -beta, -res, deadline, tt)
...
def agent(obs, config):
    ...
    reachedDepth = 2
    # Tìm kiếm sâu dần (Iterative Deepening + Principal Variation Search)
    try:
        for depth in range(reachedDepth-1, 20):
            ...
            for col in moves:
                ...
                # Root PVS: nước đầu full-window, các nước sau null-window rồi re-search nếu cần.
                if col == moves[0]:
                    score = -pvs(opp, me | new_piece, depth, -2000000, 2000000, deadline, tt)
                else:
                    score = -pvs(opp, me | new_piece, depth, -best_score - 1, -best_score, deadline, tt)
                    if best_score < score < 2000000:
                        score = -pvs(opp, me | new_piece, depth, -2000000, 2000000, deadline, tt)
